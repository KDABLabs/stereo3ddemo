...
Frustum::Frustum(const QColor& color, QNode* parent)
    ...
    , mColor(color)
{
    ...
    auto* material = new Qt3DRender::QMaterial;
    {
        ...
        colorParameter->setValue(mColor);
...
void Frustum::updateGeometry()
{
    ...
    const std::vector<QVector3D> frustumVertices{
    ...
    rawVertices.resize(frustumVertices.size() * sizeof(QVector3D));
    std::memcpy(rawVertices.data(), frustumVertices.data(), frustumVertices.size() * sizeof(QVector3D));
