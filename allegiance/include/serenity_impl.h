...
public:
	std::unique_ptr<Entity> CreateScene() const noexcept {
		...
		// Meshes
		const std::function<void(Mesh*)> generators[] = {
			[](Mesh* m) { MeshGenerators::sphereGenerator(m, 64, 32, 1.5f); },
			[](Mesh* m) {
			  MeshGenerators::planeGenerator(m, 3.0f, 3.0f, {2, 2});
			},
			[](Mesh* m) {
			  MeshGenerators::cylinderGenerator(m, 64, 32, 1.5f, 3.0f);
			},
			[](Mesh* m) {
			  MeshGenerators::coneGenerator(m, 64, 32, 1.5f, 0.5f, 2.0f);
			},
			[](Mesh* m) {
			  MeshGenerators::coneGenerator(m, 64, 32, 1.5f, 0.0f, 3.0f);
			},
			[](Mesh* m) {
			  MeshGenerators::cuboidGenerator(m, {3.0f, 1.5f, 2.0f}, {2, 2, 2});
			},
			[](Mesh* m) {
			  MeshGenerators::cuboidGenerator(m, {3.0f, 3.0f, 3.0f}, {2, 2, 2});
			},
		};
		...
		for (uint16_t i = 0, m = sizeof(generators) / sizeof(generators[0]); i < m;
			++i) {
			...
			material->setUniformBufferDataBuilder(materialDataBuilder[i]);

			SrtTransform* transform = e->createComponent<SrtTransform>();
			const float angle = float(2.0 * M_PI * (double(i) / m));
			transform->translation =
				glm::vec3{ std::cos(angle), 0, std::sin(angle) } *5.0f;

			MeshRenderer* meshRenderer = e->createComponent<MeshRenderer>();
			meshRenderer->mesh = e->createChild<Mesh>();
			meshRenderer->material = material;

			generators[i](meshRenderer->mesh());
		}
	...
	void CreateAspects(all::CameraControl* cc) {
		...
		auto camera = rootEntity->createChildEntity<StereoCamera>();
		camera->lookAt(glm::vec3(0.5f, 0.5f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f),
		...
		auto cameraFrameAction = rootEntity->createComponent<FrameAction>();

		auto rotateCamera = [=](FrameAction*) {
			const float radius = 2.0f;
			static float angle = 0.0f;
			angle += 0.01f;
			const float rad = glm::radians(angle);
			const glm::vec3 position(radius * std::cos(rad), 0.5f,
				radius * std::sin(rad));
			camera->lookAt(position + glm::vec3(-0.5f, 0.0f, 0.0f),
