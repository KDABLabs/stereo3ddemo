...
std::unique_ptr<Serenity::Entity> all::MeshLoader::load(std::filesystem::path path)
{
...
std::unique_ptr<Serenity::Mesh> all::MeshLoader::MakeMesh(const aiMesh& mesh, const aiMaterial& material)
{
...
std::unique_ptr<Serenity::SpirVShaderProgram> MakeShaderProgram(std::string shader_code)
{
    ...
    program->vertexShader = fmt::format("scene/{}.vert.spv", shader_code);
    program->fragmentShader = fmt::format("scene/{}.frag.spv", shader_code);
    return program;
}

std::unique_ptr<Serenity::Material> all::MeshLoader::MakeMaterial(const aiMaterial& material, const std::filesystem::path& model_path)
{
